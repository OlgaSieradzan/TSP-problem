...
TSP jest problemem należącym do klasy NP-trudnych, co oznacza, że wraz ze wzrostem liczby punktów (miast) liczba możliwych tras rośnie w sposób wykładniczy, a dokładne rozwiązanie staje się coraz trudniejsze do obliczenia w rozsądnym czasie. Z tego powodu opracowuje się efektywne algorytmy heurystyczne i metaheurystyczne, które umożliwiają znalezienie rozwiązań zbliżonych do optymalnych w akceptowalnym czasie.
...
<div style='text-align: justify'>
W statystykach poniżej przedstawiono najkrótszą oraz najdłuższą długość ścieżki uzyskaną przez Solver. Mamy również wyznaczoną średnią ze wszystkich wyników.
</div>
...
<div style='text-align: justify'>
**Algorytm iteracyjnej wspinaczki** to technika optymalizacyjna, stosowana do znajdowania zbliżonych do optymalnych rozwiązań problemów, które mogą mieć wiele lokalnych minimów. Wykorzystuje on podejście algorytmu wspinaczki, rozszerzone o wielokrotne losowanie różnych początkowych rozwiązań, co zwiększa szansę na znalezienie rozwiązania bliskiego globalnemu optimum.

Zasada działania algorytmu wspinaczki z multistartem do rozwiązania problemu komiwojażera:

1. Na początku każdej iteracji wybierane jest losowe rozwiązanie początkowe, które stanowi punkt wyjścia dla algorytmu wspinaczki górskiej.

2. Analizowane jest sąsiedztwo bieżącego rozwiązania w celu znalezienia trasy o najmniejszym koszcie, korzystając z wybranej metody sąsiedztwa.

3. Dla każdego sąsiedniego rozwiązania obliczany jest koszt trasy, który porównywany jest z aktualnym kosztem. Jeśli koszt sąsiedniej trasy jest niższy, 
zostaje ona zaakceptowana jako nowe rozwiązanie i proces powtarzany jest, aż nie będzie możliwe znalezienie lepszego rozwiązania.

4. Każda iteracja algorytmu jest ograniczona liczbą kroków (maksymalna liczba iteracji bez poprawy), a cały proces jest powtarzany przez określoną 
liczbę prób, aby uniknąć utknięcia w lokalnym optimum.

5. Dla każdej iteracji obliczany jest czas wykonania algorytmu, a najlepsze znalezione rozwiązanie jest zapisywane.

6. Algorytm kończy poszukiwania, gdy upłynie określony limit czasu lub po przeprowadzeniu zaplanowanej liczby iteracji.

7. Na koniec algorytm zwraca najlepszą znalezioną trasę.
</div>
...
W ramach algorytmu iteracyjnej wspinaczki badane są następujące parametry:
 ...
 - **`num_starts`** - liczba iteracji
 
 - **`neighborhood_type`** - rodzaj sąsiedztwa - metoda generowania nowych rozwiązań na podstawie aktualnego rozwiązania. W badaniu uwzględniono typy:
    ...
    - **`two-opt`** wiąże się z odwróceniem segmentu trasy między dwoma wybranymi wierzchołkami, co prowadzi do zmiany kolejności miast w tym segmencie
  
    <p style="margin-top: 10px;">
 - **`time_limit`** - limit czasowy, jaki posiada algorytm na znalezienie rozwiązania
 
 - **`max_iterations_without_improvement`** - liczba iteracji bez poprawy - liczba iteracji, po której algorytm kończy działanie, jeśli nie znaleziono lepszego rozwiązania
 
 - **`max_hill_climbing_iterations`** - liczba iteracji wspinaczki - liczba iteracji, jaką algorytm może wykonać w jednej próbie wspinaczki
    </p>
...
```{r, echo=FALSE}
 ...
 #     sheet = "num_starts")
 # IHC_move_type <- read_excel("C:/Users/Justyna/source/repos/Projekt_IO/TSP-problem/IHC.xlsx", 
 #                                  sheet = "neighborhood_type")
 # IHC_max_hill_climbing_iterations <- read_excel("C:/Users/Justyna/source/repos/Projekt_IO/TSP-problem/IHC.xlsx", 
 #     sheet = "max_hill_climbing_iterations")
 # IHC_max_iterations_without_improvement <- read_excel("C:/Users/Justyna/source/repos/Projekt_IO/TSP-problem/IHC.xlsx", 
 #     sheet = "max_iterations_without_improvem")
 # IHC_time_limit <- read_excel("C:/Users/Justyna/source/repos/Projekt_IO/TSP-problem/IHC.xlsx", 
 #     sheet = "time_limit")
 
 IHC_clasic <- read_excel("C:/Users/user/Downloads/IHC.xlsx", 
                               sheet = "Basic")
 IHC_max_iterations <- read_excel("C:/Users/user/Downloads/IHC.xlsx", 
     sheet = "num_starts")
 IHC_move_type <- read_excel("C:/Users/user/Downloads/IHC.xlsx", 
                                  sheet = "neighborhood_type")
 IHC_max_hill_climbing_iterations <- read_excel("C:/Users/user/Downloads/IHC.xlsx", 
     sheet = "max_hill_climbing_iterations")
 IHC_max_iterations_without_improvement <- read_excel("C:/Users/user/Downloads/IHC.xlsx", 
     sheet = "max_iterations_without_improvem")
 IHC_time_limit <- read_excel("C:/Users/user/Downloads/IHC.xlsx", 
     sheet = "time_limit")
```
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji (TSP_48)**
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji (TSP_76)**
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji (TSP_127)**
...
ggplot(IHC_move_type_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ rodzaju sąsiedztwa na długość ścieżki (TSP_48)",
    x = "Rodzaj sąsiedztwa",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od rodzaju sąsiedztwa (TSP_48)**
...
ggplot(IHC_move_type_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ rodzaju sąsiedztwa na długość ścieżki (TSP_76)",
    x = "Rodzaj sąsiedztwa",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od rodzaju sąsiedztwa (TSP_76)**
...
ggplot(IHC_move_type_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ rodzaju sąsiedztwa na długość ścieżki (TSP_127)",
    x = "Rodzaj sąsiedztwa",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od rodzaju sąsiedztwa (TSP_127)**
...
Poniżej zbadany został wpływ limitu czasowego na długość ścieżki:
...
ggplot(IHC_time_limit_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ limitu czasowego na długość ścieżki (TSP_48)",
    x = "Limit czasowy",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od limitu czasowego (TSP_48)**
...
ggplot(IHC_time_limit_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ limitu czasowego na długość ścieżki (TSP_76)",
    x = "Limit czasowy",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od limitu czasowego (TSP_76)**
...
ggplot(IHC_time_limit_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ limitu czasowego na długość ścieżki (TSP_127)",
    x = "Limit czasowy",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od limitu czasowego (TSP_127)**
...
ggplot(IHC_max_iterations_without_improvement_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby iteracji bez poprawy na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji bez poprawy (TSP_48)**
...
ggplot(IHC_max_iterations_without_improvement_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby iteracji bez poprawy na długość ścieżki (TSP_76)",
    x = "Liczba iteracji bez poprawy",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji bez poprawy (TSP_76)**
...
ggplot(IHC_max_iterations_without_improvement_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby iteracji bez poprawy na długość ścieżki (TSP_127)",
    x = "Liczba iteracji bez poprawy",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji bez poprawy (TSP_127)**
...
Poniżej zbadany został wpływ liczby iteracji wspinaczki na długość ścieżki:
...
ggplot(IHC_max_hill_climbing_iterations_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby iteracji wspinaczki na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji wspinaczki (TSP_48)**
...
<div style='text-align: justify'>
Wyniki są dość zbliżone dla każdej wartości parametru. Średnia i mediana wahają się wokół podobnych wartośći.
</div>
...
ggplot(IHC_max_hill_climbing_iterations_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby iteracji wspinaczki na długość ścieżki (TSP_76)",
    x = "Liczba iteracji wspinaczki",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji wspinaczki (TSP_76)**
...
<div style='text-align: justify'>
Tutaj natomiast widzimy delikatny spadek mediany i średniej wraz ze wzrostem liczby iteracji wspinaczki. Jednak nie jest on bardzo znaczący.
</div>
...
ggplot(IHC_max_hill_climbing_iterations_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby iteracji wspinaczki na długość ścieżki (TSP_127)",
    x = "Liczba iteracji wspinaczki",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od liczby iteracji wspinaczki (TSP_127)**
...
<div style='text-align: justify'>
Dla TSP_127 widać na początku dość intensywny spadek mediany i średniej, jednak później wartości wzrastają.

Ciężko stwierdzić czy isteniej tu jakaś zależność. Dla TSP_76 i TSP_127 widać na początku spadek mediany i średniej. Dla TSP_48 nie widać wpływu tego parametru.
</div>
...
Najlepsze wyniki dla **TSP_48** przy zmianie poszczególnych parametrów:
...
* `Liczba iteracji wspinaczki` - 10690 (liczba iteracji wspinaczki - 1000)

Dla TSP_48 przy użyciu algorytmu iteracyjnej wspinaczki z multistartem udało się uzyskać zdecydownie lepsze wyniki niż ten uzyskany przez Solver (10868).

<br>
Najlepsze wyniki dla **TSP_76** przy zmianie poszczególnych parametrów:
...
* `Liczba iteracji wspinaczki` - 109036.8	(liczba iteracji wspinaczki - 750)

Dla TSP_76 przy użyciu algorytmu iteracyjnej wspinaczki z multistartem również udało się uzyskać zdecydowanie lepsze wyniki niż ten uzyskany przez Solver (118293.5).

<br>
Najlepsze wyniki dla **TSP_127** przy zmianie poszczególnych parametrów:
...
* `Liczba iteracji wspinaczki` - 120273.5	(liczba iteracji wspinaczki - 1000)

Dla TSP_127 przy użyciu algorytmu iteracyjnej wspinaczki z multistartem również są lepsze wyniki niż ten uzyskany przez Solver (140217.6).
...
<div style='text-align: justify'>
**Algorytm symulowanego wyżarzania** (ang. Simulated Annealing, SA) jest iteracyjną metodą optymalizacji, która rozpoczyna się od losowego rozwiązania i w każdej iteracji wprowadza zmiany w obrębie sąsiedztwa. Inspiracją dla algorytmu SA był proces metalurgiczny, w którym metal w wysokich temperaturach łatwiej poddaje się obróbce plastycznej. W miarę obniżania temperatury, zmiany kształtu stają się coraz trudniejsze. Podobnie, w algorytmie symulowanego wyżarzania na początku akceptowane są ruchy prowadzące do gorszych rozwiązań z relatywnie dużym prawdopodobieństwem, gdy temperatura jest wysoka. Z upływem iteracji oraz spadkiem temperatury, prawdopodobieństwo akceptacji gorszych rozwiązań maleje, co skutkuje tym, że algorytm niemal zawsze odrzuca pogarszające ruchy. Mechanizm ten ma na celu unikanie utknięcia w ekstremach lokalnych. W każdej iteracji generowane są losowe ruchy, a ich liczba jest parametryzowana w danej implementacji. 

Zasada działania algorytmu symulowanego wyżarzania do rozwiązania problemu komiwojażera:

1. Na początku losowo generowana jest początkowa trasa, która reprezentuje kolejność odwiedzania miast. Ta trasa jest traktowana jako bieżące rozwiązanie i wyznaczany jest koszt tej trasy.

2. Ustalana jest początkowa wartość temperatury, która kontroluje prawdopodobieństwo akceptacji gorszych rozwiązań w trakcie przeszukiwania.

3. W każdej iteracji generowany jest sąsiad bieżącego rozwiązania w oparciu o wybrany typ ruchu.

4. Dla nowego rozwiązania obliczany jest koszt, a następnie różnica kosztów między bieżącym a sąsiednim rozwiązaniem. 

5. Jeśli nowa trasa jest lepsza, algorytm akceptuje ją jako nowe bieżące rozwiązanie. Jeśli jest gorsza, algorytm może ją zaakceptować z prawdopodobieństwem zależnym od różnicy kosztów i aktualnej temperatury.

6. Jeśli bieżące rozwiązanie jest lepsze od dotychczasowego najlepszego rozwiązania, algorytm zapisuje je jako nowe najlepsze rozwiązanie.

7. Po każdej iteracji temperatura jest zmniejszana poprzez pomnożenie jej przez współczynnik chłodzenia. Obniżanie temperatury zmniejsza prawdopodobieństwo akceptacji gorszych rozwiązań.

8. Algorytm kończy działanie, gdy spełniony zostanie jeden z warunków zakończenia: liczba iteracji osiągnie maksymalną wartość, temperatura spadnie poniżej minimalnej wartości, lub czas działania przekroczy ustalony limit.

9. Po zakończeniu działania algorytm zwraca najlepsze znalezione rozwiązanie.
</div>
...
W ramach algorytmu symulowanego wyżarzania badane są następujące parametry:
 ...
 - **`move_type`** - rodzaj sąsiedztwa - metoda generowania nowych rozwiązań na podstawie aktualnego rozwiązania. W badaniu uwzględniono typy:
    ...
    <p style="margin-top: 10px;">
 - **`initial_temperature`** -  początkowa wartość temperatury, która kontroluje akceptację gorszych rozwiązań.
 
 - **`cooling_rate`** - współczynnik chłodzenia, określający szybkość obniżania temperatury w kolejnych iteracjach
 
 - **`max_iterations`** - liczba iteracji, po których algorytm kończy działanie

 - **`min_temperature`** -minimalna wartość temperatury, poniżej której algorytm kończy działanie
 
 - **`time_limit`** - limit czasowy
    </p>
    
```{r echo=FALSE, include=FALSE}
  SA_clasic <- read_excel("C:/Users/user/Downloads/SA.xlsx", 
      sheet = "Basic")
  SA_move_type <- read_excel("C:/Users/user/Downloads/SA.xlsx", 
      sheet = "move_type")
...
<div style='text-align: justify'>
Uzyskane czasy przy podstawowych parametrach są bardzo niskie, nie wynoszą nawet sekundy. W Solverze w każdym z przypadków były one zdecydowanie wyższe.
</div>
...
<div style='text-align: justify'>
Dla TSP_76 pojawiła się analogiczna sytuacja. Dla `2-opt` wyniki są najlepsze. Uzyskana minimalna długość ścieżki dla tej wartości parametru jest znacząco lepsza od Solvera (118293.5). Ponownie najgorzej wypada `swap`. Średnie różnią się znacząco w zależności od typu ruchu.
</div>
...
ggplot(SA_initial_temp_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ temperatury początkowej na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od wartości temperatury początkowej (TSP_48)**
...
ggplot(SA_initial_temp_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ temperatury początkowej na długość ścieżki (TSP_76)",
    x = "Temperatura początkowa",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
**Statystyki długości ścieżki w zależności od wartości temperatury początkowej (TSP_76)**
...
<div style='text-align: justify'>
Dla TSP_76 pojawiają się już większe wahania na początku, ale nadal nie widać, żeby w zależności od temperatury mediana długości ścieżki się znacząco zmieniała. Nie mamy tutaj też bardzo dużych odstępstw tzn. średnie i mediany są dość zbliżone do siebie. Otrzymane wyniki są bardzo dobre i zdecydowanie lepsze od tych z Solvera.
</div>
...
ggplot(SA_initial_temp_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ temperatury początkowej na długość ścieżki (TSP_127)",
    x = "Temperatura początkowa",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od wartości temperatury początkowej (TSP_127)**
...
ggplot(SA_cooling_rate_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prędkości chłodzenia na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od prędkości chłodzenia (TSP_48)**
...
<div style='text-align: justify'>
Z wykresu bardzo dobrze widać jak mediana długości ścieżki maleje wraz ze wzrostem prędkości chłodzenia. Średnia również wykazuje takie zachowanie. Jednak pod koniec mediana i średnia długości ścieżki nie maleją tak szybko, stają się wręcz zbliżone (0.94-0.98).
</div>
...
ggplot(SA_cooling_rate_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prędkości chłodzenia na długość ścieżki (TSP_76)",
    x = "Prędkość chłodzenia",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od prędkości chłodzenia (TSP_76)**
...
ggplot(SA_cooling_rate_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prędkości chłodzenia na długość ścieżki (TSP_127)",
    x = "Prędkość chłodzenia",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od prędkości chłodzenia (TSP_127)**
...
ggplot(SA_min_temp_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ minimalnej temperatury na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od wartości temperatury minimalnej (TSP_48)**
...
ggplot(SA_min_temp_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ minimalnej temperatury na długość ścieżki (TSP_76)",
    x = "Minimalna temperatura",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od wartości temperatury minimalnej (TSP_76)**
...
ggplot(SA_min_temp_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ minimalnej temperatury na długość ścieżki (TSP_127)",
    x = "Minimalna temperatura",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od wartości temperatury minimalnej (TSP_127)**
...
* `Rodzaj sąsiedztwa` - 10928 (rodzaj sąsiedztwa - 2-opt)
...
Dla TSP_48 przy użyciu algorytmu symulowanego wyżarzania udało się uzyskać parę lepszych wyników niż ten uzyskany przez Solver (10868).
...
* `Rodzaj sąsiedztwa` - 113460.2 (rodzaj sąsiedztwa - 2-opt)

* `Temperatura początkowa` - 113094.6 (temperatura początkowa - 50 tys.)

* `Prędkość chłodzeni`a - 114398.9 (prędkość chłodzenia - 0.97)
...
* `Rodzaj sąsiedztwa` - 148832.8 (rodzaj sąsiedztwa - 2-opt)
...
Dla TSP_127 przy użyciu algorytmu symulowanego wyżarzania nie udało się uzyskać lepszych wyników niż ten uzyskany przez Solver (140217.6). 
...
Zasada działania algorytmu przeszukiwania tabu do rozwiązania problemu komiwojażera:

1. Generowane jest początkowe rozwiązanie oraz obliczany jego koszt.

2. Inicjalizowana jest lista tabu z ograniczoną długością. Jeśli lista osiągnie maksymalną długość, najstarsze rozwiązanie zostaje z niej usunięte.

3. W każdej iteracji algorytmu generowane jest sąsiedztwo, które zawiera rozwiązania uzyskane poprzez zastosowanie jednego z trzech typów ruchów. Sąsiedztwo jest generowane na podstawie bieżącego rozwiązania.

4. Każde rozwiązanie w sąsiedztwie jest oceniane pod kątem kosztu. Spośród tych rozwiązań wybierany jest najlepszy sąsiad, który nie jest na liście tabu,a którego koszt jest mniejszy niż dotychczasowego najlepszego rozwiązania.

5. Jeżeli znaleziony najlepszy sąsiad nie jest na liście tabu, staje się on nowym rozwiązaniem bazowym i jest dodawany do listy tabu.

6. Jeśli znaleziono lepsze rozwiązanie, aktualizowane są zarówno bieżące rozwiązanie, jak i najlepsze rozwiązanie, jeśli nowe jest lepsze od dotychczasowego.

7. Algorytm kończy działanie, gdy zostanie osiągnięty limit czasu, maksymalna liczba iteracji lub maksymalna liczba iteracji bez poprawy rozwiązania.

8. Po zakończeniu działania, algorytm zwraca najlepsze znalezione rozwiązanie.
</div>
...
W ramach algorytmu przeszukiwania tabu badane są następujące parametry:
 ...
 - **`move_type`** - rodzaj sąsiedztwa - metoda generowania nowych rozwiązań na podstawie aktualnego rozwiązania. W badaniu uwzględniono typy:
    ...
    <p style="margin-top: 10px;">
 - **`tabu_list_length`** - długość listy tabu - liczba wykonanych ruchów, które nie mogą być ponownie wykorzystanie przez określoną liczbę iteracji
 
 - **`max_iterations`** - liczba iteracji, po której algorytm kończy działanie
 
  - **`max_no_improve`** - liczba iteracji bez poprawy - liczba iteracji, po której algorytm kończy działanie, jeśli nie znaleziono lepszego rozwiązania
...
```{r echo=FALSE, include=FALSE}
 ...
 #     sheet = "time_limit")
  ...
  TS_clasic <- read_excel("C:/Users/user/Downloads/TS.xlsx", 
      sheet = "Basic")
  TS_move_generator <- read_excel("C:/Users/user/Downloads/TS.xlsx", 
      sheet = "move_generator")
  TS_list_length <- read_excel("C:/Users/user/Downloads/TS.xlsx", 
      sheet = "tabu_list_length")
  TS_max_iterations <- read_excel("C:/Users/user/Downloads/TS.xlsx", 
      sheet = "max_iterations")
  TS_max_no_improve <- read_excel("C:/Users/user/Downloads/TS.xlsx", 
      sheet = "max_no_improve")
  TS_time_limit <- read_excel("C:/Users/user/Downloads/TS.xlsx", 
      sheet = "time_limit")
```
...
Poniżej zbadany został wpływ rodzaju sasiedztwa na długość ścieżki:
...
ggplot(TS_move_generator_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ rodzaju sasiedztwa na długość ścieżki (TSP_48)",
    x = "Rodzaj sąsiedztwa",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<div style='text-align: justify'>
Pojawiła nam się tutaj również zależność pomiędzy rodzajem sąsiedztwa, a medianą i średnią długością ścieżki. Najlepsze wyniki uzyskiwane są dla rodzaju sąsiedtwa `2-opt`, a najgorsze dla `swap`. Wyniki dla `2-opt` charakteryzują się małą wariancją (rozkład jest wąski), co wskazuje na niewielkie różnice w długościach ścieżek. 
</div>
...
ggplot(TS_move_generator_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ rodzaju sasiedztwa na długość ścieżki (TSP_76)",
    x = "Rodzaj sąsiedztwa",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
ggplot(TS_move_generator_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ rodzaju sasiedztwa na długość ścieżki (TSP_127)",
    x = "Rodzaj sąsiedztwa",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<div style='text-align: justify'>
Dla TSP_127 analogiczna sytuacja. Jednak pomimo różnych rodzajów sąsiedztwa, wyniki nadal są zdecydowanie gorsze.

Na podstawie analizy widać dużą zależność długości ścieżki od rodzju sąsiedztwa. W każdym z przypadków, najlepsze wyniki uzyskano dla `2-opt`, a najgorsze dla `swap`. Ponadto rozkłady wyników dla `2-opt` charakteryzują się małą wariancją w odróżnieniu do pozostałych.
</div>
...
ggplot(TS_list_length_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ długości listy tabu na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od długości listy tabu (TSP_48)**
...
ggplot(TS_list_length_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ długości listy tabu na długość ścieżki (TSP_76)",
    x = "Długość listy tabu",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od długości listy tabu (TSP_76)**
...
ggplot(TS_list_length_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ długości listy tabu na długość ścieżki (TSP_127)",
    x = "Długość listy tabu",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od długości listy tabu (TSP_127)**
...
<div style='text-align: justify'>
Natomiast dla TSP_127 widać dobrze, że wraz ze wzrostem długości listy, maleje średnia i mediana długości ścieżki. Przy tak dużej liczbie miast (127), mała lista tabu nie spełnia dobrze swojej roli i często algorytm wraca do rozwiązań już odwiedzonych. 

Pojawia się tu pewna zależność między długością listy tabu a długością ścieżki. Jednak dużo też zależy od liczby miast w problemie komiwojażera. Od niej powinna być uzależniona długość listy tabu. Dla TSP_48 można byłby sprawdzić więcej wartości tego parametru, może wtedy ta zależność byłaby tam bardziej widoczna. Natomiast ciekawy przypadek jest dla TSP_76. Ciężko tam stwierdzić, aby jakakolwiek zależność istniała.
</div>
...
<div style='text-align: justify'>
Tutaj pojawia się podobna zależność co w przypadku TSP_48. Średnia i mediana rosną w porównaniu do tych uzyskanych dla liczby iteracji = 500. Jednak wyniki również w tym przypadku są zdecydowanie lepsze niż te uzyskane przez Solver(140217.6).

Ciężko więc stwierdzić, czy występuje tu jakaś zależność. W 2/3 przypadków wzrost liczby iteracji powodują wzrost średniej i mediany.
</div>
...
<div style='text-align: justify'>
Sytaucja jest tutaj podobna. Jednak już przy liczbie iteracji bez poprawy = 150 mamy wzrost mediany i średniej. Dla TSP_76 już od tej wartości algorytm może zbyt długo poszukiwać lepszego rozwiązania. 
</div>
...
<div style='text-align: justify'>
Dla TSP_127 na początku widać duży spadek wartości mediany i średniej. Nie było to w widoczne w poprzednich TSP w takim stopniu. Może to się wiązać z tym, że TSP_127 ma największą liczbę miast, więc limit czasowy = 60s był zbyt mały dla niego.
...
* `Rodzaj sąsiedztwa` - 10941 (rodzaj sąsiedztwa - 2-opt)
...
Dla TSP_48 przy użyciu algorytmu tabu search udało się uzyskać parę lepszych wyników niż ten uzyskany przez Solver (10868).
...
* `Rodzaj sąsiedztwa` - 128558.9 (rodzaj sąsiedztwa - 2-opt)
...
* `Rodzaj sąsiedztwa` - 200281.5 (rodzaj sąsiedztwa - 2-opt)
...
Zasada działania algorytmu genetycznego do rozwiązania problemu komiwojażera:

1. Na początku inicjalizowana jest populacja z losowymi permutacjami.

2. Wybór osobników do krzyżowania odbywa się na podstawie jednej z metod selekcji: turniejowej lub ruletki, które przydzielają większą szansę na wybór osobnikom o lepszym dopasowaniu.

3. Algorytm tworzy nowe rozwiązania (dzieci) przez krzyżowanie par rodziców za pomocą jednej z metod: order lub PMX.

4. Mutacja jest wykonywana zgodnie z wybranym typem.

5. Po każdym pokoleniu algorytm ocenia rozwiązania, wybierając najlepsze na podstawie minimalnej odległości (kosztu trasy).

6. Algorytm powtarza proces selekcji, krzyżowania i mutacji przez wiele pokoleń, stale dążąc do poprawy wyników, aż osiągnie ustaloną liczbę pokoleń lub osiągnie optymalne rozwiązanie.
</div>
...
W ramach algorytmu genetycznego badane są następujące parametry:
 ...
 - **`population_size`** - wielkość populacji - liczba osobników w populacji, które będą brane pod uwagę w danej iteracji algorytmu
 
 - **`mutation_rate`** - prawdopodobieństwo mutacji - prawdopodobieństwo z jakim nastąpi mutacja genów w osobnikach
 
 - **`crossover_rate`** - prawdopodobieństwo krzyżowania - prawdopodobieństwo z jakim nastąpi krzyżowanie osobników w populacji
 
 - **`selection_method`** - metoda doboru rodziców - sposób wyboru najlepszych osobników do reprodukcji:
 
   - `tournament` - metoda turniejowa - losowo wybierane jest k osobników z populacji. Następnie spośród nich wybierany jest 
osobnik o najlepszym wyniku.
   
   - `roulette` - metoda ruletki - każdemu osobnikowi w populacji przydzielane jest prawdopodobieństwo wyboru proporcjonalne do jego wyniku. Następnie, losowana jest liczba z przedziału [0, 1], a na jej podstawie wybierany jest osobnik: im większe prawdopodobieństwo, tym większa szansa, że jego wynik odpowiada wylosowanej liczbie
   
   <p style="margin-top: 10px;">
 - **`crossover_method`** - metoda krzyżowania - sposób łączenia genów (tras) rodziców w celu stworzenia potomków:
 
   - `order` - z pierwszego rodzica wybierany jest losowy segment miast, który jest kopiowany do potomka w to samo miejsce, pozostałe miasta są uzupełniane w potomku zgodnie z ich kolejnością wystąpienia w drugim rodzicu
   
   - `pmx` - dziedziczenie części trasy od każdego z rodziców przy zachowaniu oryginalnej kolejności odwiedzonych miast, jednocześnie eliminując duplikaty w nowo powstałym rozwiązaniu 
   </p>

- **`generations`** - liczba pokoleń, czyli liczba iteracji algorytmu
   
   <p style="margin-top: 10px;">
 - **`mutation_method`** - metoda mutacji - metoda generowania nowych rozwiązań na podstawie aktualnego rozwiązania. W badaniu uwzględniono typy:
 
    - `swap` nowe rozwiązanie powstaje poprzez zamianę miejscami dwóch genów w bieżącej permutacji trasy
    
    - `insertion` polega na wyborze jednego genu z danej permutacji, usunięciu go z jego pierwotnej pozycji, a następnie wstawieniu go w inne, wybrane miejsce w tej samej permutacji
    
    - `inversion` odwrócenie kolejności genów w wybranym segmencie permutacji
    </p>
...
```{r echo=FALSE, include=FALSE}
 ...
 GA_klasyk <- read_excel("C:/Users/user/Downloads/GA.xlsx", 
     sheet = "Basic")
 GA_population_size <- read_excel("C:/Users/user/Downloads/GA.xlsx", 
     sheet = "population_size")
 GA_mutation_rate <- read_excel("C:/Users/user/Downloads/GA.xlsx", 
     sheet = "mutation_rate")
 GA_crossover_rate <- read_excel("C:/Users/user/Downloads/GA.xlsx", 
     sheet = "crossover_rate")
 GA_generations <- read_excel("C:/Users/user/Downloads/GA.xlsx", 
     sheet = "generations")
 GA_selection_method <- read_excel("C:/Users/user/Downloads/GA.xlsx", 
     sheet = "selection_method")
 GA_crossover_method <- read_excel("C:/Users/user/Downloads/GA.xlsx", 
     sheet = "crossover_method")
 GA_mutation_method <- read_excel("C:/Users/user/Downloads/GA.xlsx", 
     sheet = "mutation_method")
```
...
ggplot(GA_population_size_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ wielkości populacji na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od wielkości populacji (TSP_48)**
...
ggplot(GA_population_size_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ wielkości populacji na długość ścieżki (TSP_76)",
    x = "Wielkość populacji",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od wielkości populacji (TSP_76)**
...
<div style='text-align: justify'>
Dla TSP_76 dla parametru = 5 średnia i mediana są również największe w porównaniu do pozostałych wyników. Natępnie widać, że wraz ze wzrostem liczby populacji, maleje średnia i mediana.
</div>
...
ggplot(GA_population_size_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ wielkości populacji na długość ścieżki (TSP_127)",
    x = "Wielkość populacji",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od wielkości populacji (TSP_127)**
...
<div style='text-align: justify'>
Tutaj pojawia się analogiczna sytuacja jak dla TSP_76. Gdy zwiększyła się liczba miast, bardziej widoczna stała się zależność, że wraz ze wzrostem liczby populacji, maleje średnia i mediana długości ścieżki.
...
ggplot(GA_mutation_rate_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prawdopodobieństwa mutacji na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od prawdopodobieństwa mutacji (TSP_48)**
...
ggplot(GA_mutation_rate_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prawdopodobieństwa mutacji na długość ścieżki (TSP_76)",
    x = "Prawdopodobieństwo mutacji",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od prawdopodobieństwa mutacji (TSP_76)**
...
ggplot(GA_mutation_rate_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prawdopodobieństwa mutacji na długość ścieżki (TSP_127)",
    x = "Prawdopodobieństwo mutacji",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od prawdopodobieństwa mutacji (TSP_127)**
...
<div style='text-align: justify'>
Analogicznie dla TSP_127. Chociaż średnia w tym przypadku nawet dla parametru = 0.2 jest wyższa niż ta uzyskana w Solverze.

W każdym z przypadków bardzo dobrze widoczna jest zależność, że wraz ze wzrostem prawdopodobieństwa mutacji, maleje długość ścieżki. Mutowanie pozwala na eksplorowanie większej liczby rozwiązań. Niskie wartości parametru powodują, że osobniki w populacji są do siebie dość zbliżone, dlatego długości ścieżki są wtedy większe.
</div>
...
ggplot(GA_crossover_rate_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prawdopodobieństwa krzyżowania na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od prawdopodobieństwa krzyżowania (TSP_48)**
...
ggplot(GA_crossover_rate_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prawdopodobieństwa krzyżowania na długość ścieżki (TSP_76)",
    x = "Prawdopodobieństwo krzyżowania",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od prawdopodobieństwa krzyżowania (TSP_76)**
...
ggplot(GA_crossover_rate_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ prawdopodobieństwa krzyżowania na długość ścieżki (TSP_127)",
    x = "Prawdopodobieństwo krzyżowania",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od prawdopodobieństwa krzyżowania (TSP_127)**
...
ggplot(GA_generations_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby pokoleń na długość ścieżki (TSP_48)",
    x = "Liczba pokoleń",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od liczby pokoleń (TSP_48)**
...
ggplot(GA_generations_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby pokoleń na długość ścieżki (TSP_76)",
    x = "Liczba pokoleń",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od liczby pokoleń (TSP_76)**
...
ggplot(GA_generations_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ liczby pokoleń na długość ścieżki (TSP_127)",
    x = "Liczba pokoleń",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od liczby pokoleń (TSP_127)**
...
Poniżej zbadany został wpływ metody doboru rodziców na długość ścieżki:
...
ggplot(GA_selection_method_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody doboru rodziców na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od metody doboru rodziców (TSP_48)**
...
<div style='text-align: justify'>
Dobieranie rodziców metodą ruletki zdecydowanie sie nie sprawdza. Wyniki nawet nie są zbliżone do tych osiągniętych przy metodzie turniejowy.
</div>
...
ggplot(GA_selection_method_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody doboru rodziców na długość ścieżki (TSP_76)",
    x = "Metoda doboru rodziców",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od metody doboru rodziców (TSP_76)**
...
Tutaj sytuacja analogiczna. Różnica pomiędzy wynikami jest diametralna.
...
ggplot(GA_selection_method_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody doboru rodziców na długość ścieżki (TSP_127)",
    x = "Metoda doboru rodziców",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od metody doboru rodziców (TSP_127)**
...
Poniżej zbadany został wpływ metody krzyżowania na długość ścieżki:
...
ggplot(GA_crossover_method_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody krzyżowania na długość ścieżki (TSP_48)",
...
<br>
**Statystyki długości ścieżki w zależności od metody krzyżowania (TSP_48)**
...
ggplot(GA_crossover_method_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody krzyżowania na długość ścieżki (TSP_76)",
    x = "Metoda krzyżowania",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od metody krzyżowania (TSP_76)**
...
ggplot(GA_crossover_method_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody krzyżowania na długość ścieżki (TSP_127)",
    x = "Metoda krzyżowania",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od metody krzyżowania (TSP_127)**
...
Poniżej zbadany został wpływ metody mutacji na długość ścieżki:
...
ggplot(GA_mutation_method_tsp48, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody mutacji na długość ścieżki (TSP_48)",
    x = "Metoda mutacji",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od metody mutacji (TSP_48)**
...
ggplot(GA_mutation_method_tsp76, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody mutacji na długość ścieżki (TSP_76)",
    x = "Metoda mutacji",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od metody mutacji (TSP_76)**
...
ggplot(GA_mutation_method_tsp127, aes(x = as.factor(PARAMETR), y = WYNIK_1)) +
  geom_boxplot(fill = "pink") +
  labs(
    title = "Wpływ metody mutacji na długość ścieżki (TSP_127)",
    x = "Metoda mutacji",
    y = "Długość ścieżki"
  ) +
  theme_light()
...
<br>
**Statystyki długości ścieżki w zależności od metody mutacji (TSP_127)**
...
Dla TSP_76 przy użyciu algorytmu mrówkowego udało się uzyskać kilka lepszych wyników niż ten uzyskany przez Solver (118293.5).
...
<div style='text-align: justify'>
***Algorytm mrówkowy*** (ang. Ant Colony Optimization, ACO) został stworzony na podstawie obserwacji życia mrówek podczas szukania optymalnej drogi między mrowiskiem a źródłem pożywienia. Mrówki w środowisku naturalnym poruszają się losowo, ale zostawiają na swojej drodze feromony, które pomagają innym mrówkom znaleźć najkrótszą ścieżkę. Z czasem feromony parują (ich wpływ maleje), co pozwala wyeliminować nieoptymalne ścieżki. Algorytm ten dobrze sprawdza się w rozwiązywaniu problemu komiwojażera.
...
```{r, echo=FALSE, include=FALSE}
 ...
 klasyk_wyniki <- read_excel("C:/Users/user/Downloads/ACO.xlsx", 
     sheet = "Klasyk_wyniki")
 num_ants <- read_excel("C:/Users/user/Downloads/ACO.xlsx", 
     sheet = "num_ants")
 max_it <- read_excel("C:/Users/user/Downloads/ACO.xlsx", 
     sheet = "max_it")
 alpha <- read_excel("C:/Users/user/Downloads/ACO.xlsx", 
     sheet = "alpha")
 beta <- read_excel("C:/Users/user/Downloads/ACO.xlsx", 
     sheet = "beta")
 rho <- read_excel("C:/Users/user/Downloads/ACO.xlsx", 
     sheet = "rho")
```
...
<br>
**Statystyki długości ścieżki w zależności od liczby mrówek (TSP_48)**
...
<div style='text-align: justify'>
Dla TSP_48 możemy zobaczyć zobaczyć z wykresów pudełkowych, że wraz ze wzrostem liczby mrówek, która bierze udział w poszukiwaniu optymalnej trasy, mediana długości ścieżki zdecydowanie się obniża. Średnia również widocznie maleje. Najniższa jest dla liczby mrówek równej 84. 
</div>
...
<br>
**Statystyki długości ścieżki w zależności od liczby mrówek (TSP_76)**
...
<br>
**Statystyki długości ścieżki w zależności od liczby mrówek (TSP_127)**
...
<div style='text-align: justify'>
W przypadku zmian liczby iteracji, nie widać żadnej zależności. Mediana długości ścieżki raz rośnie, raz maleje. Jedyny duży spadek widać na początku. Dla parametrów powyżej wartości 100, mediany już są niższe, średnia również. Poza tym liczba iteracji nie wpływa znacząco na wyniki. 
</div>
...
Wpływ liczby iteracji wydaje się być częściowo uzależniony od liczby miast. Gdy wynosiła ona 48 nie widać było zależności. Wraz z zwiększeniem liczby miast stawała się ona bardziej wyraźna. Jednak pojawiały się wtedy pewne wahania.
</div>
...
Poniżej został zbadany wpływ parametru alfa na długość ścieżki:
...
<br>
**Statystyki długości ścieżki w zależności od wartości parametru alfa (TSP_48)**
...
<br>
**Statystyki długości ścieżki w zależności od wartości parametru alfa (TSP_76)**
...
<div style='text-align: justify'>
Tutaj sytuacja jest praktycznie identyczna. Dla alpha = 0 wyniki są najgorsze. Natomiast wraz ze wzrostem parametru (alpha > beta) wyniki pogarszają się do momentu gdzie nie ma już znaczącej różnicy w wynikach.
</div>
...
<br>
**Statystyki długości ścieżki w zależności od wartości parametru alfa (TSP_127)**
...
Wartość parametru alpha istotnie wpływa na długość ścieżki, a tak naprawdę to jego różnica w stosunku do parametru beta. Gdy jest ona odpowiednia, wyniki są najlepsze (średnia i mediana długości ścieżki sa najniższe). W każdym przypadku najlepsze wyniki uzyskano dla alpha = 1 (beta = 2). Uzyskane wyniki nie są lepsze od tych uzyskanych za pomocą Solvera. Średnie są porównywalne bądź gorsze.
</div>
...
Poniżej został zbadany wpływ parametru beta na długość ścieżki:
...
<br>
**Statystyki długości ścieżki w zależności od wartości parametru beta (TSP_48)**
...
<br>
**Statystyki długości ścieżki w zależności od wartości parametru beta (TSP_76)**
...
<div style='text-align: justify'>
Tutaj mamy analogiczną sytuację do poprzedniej. Dla parametru beta = 0 wyniki są najgorsze. Następnie już tylko nieznacznie się wahają. 
</div>
...
<br>
**Statystyki długości ścieżki w zależności od wartości parametru beta (TSP_127)**
...
<div style='text-align: justify'>
Dla TSP_127 wzrost wartości parametru powoduje poprawę w wynikach. Średnia i mediana długości ścieżki maleją, aż do momentu gdzie już nie uzyskujemy poprawy i wyniki tylko nieznacznie wahają się wokół podobnej wartości. W tym przypadku wzrost wartości parametru beta spowodował największą poprawę.

Współczynnik beta znacząco wpływa na długość ścieżki. Jednak tak jak było wspomniane przy parametrze alpha, największe znaczenie ma różnica między tymi dwoma parametrami.
</div>
...
<br>
**Statystyki długości ścieżki w zależności od wartości rho (TSP_48)**
...
<div style='text-align: justify'>
Wyniki średniej i mediany długości ścieżki wahają się wokół podobnych wartości niezależnie od wartości rho. Jedynie znacząca różnica jest dla rho = 0.9, średnia i mediana są zdecydowanie wyższe. Uzyskane średnie są zdecydowanie wyższe niż te otrzymane przez Solver.
</div>
...
<br>
**Statystyki długości ścieżki w zależności od wartości rho (TSP_76)**
...
<div style='text-align: justify'>
Dla TSP_76 widzimy natomiast, że różnice w średniej i medianie długości ścieżki są zdecydowanie największe w przypadku skrajnych wartości rho. Bardzo niskie wartości tego parametru oznaczają długotrwałe utrzymywanie się feromonów (bardzo wolne odparowywanie) co prowadzi do nadmiernej eksploatacji jednej trasy i ogranicza zdolność algorytmu do eksploracji nowych, potencjalnie lepszych tras. Natomiast zbyt wysokie wartości powodują bardzo szybkie odparowywanie feromonów co sprawia, że ślady na trasach szybko zanikają. W tej sytuacji algorytm prawie w ogóle nie korzysta z informacji zgromadzonych przez inne mrówki, ponieważ ślady feromonowe znikają zanim inne mrówki zdążą je wykorzystać. W związku z tym dla wartości rho, które oscylują wokół 0.5 wyniki są lepsze, ale jednocześnie zbliżone do siebie.
</div>
...
<br>
**Statystyki długości ścieżki w zależności od wartości rho (TSP_127)**
...
* `Parametr alfa` - 11414	(alpha - 1)

* `Parametr beta` - 11313 (beta - 6)
...
* `Parametr alfa` - 125278.5	(alpha - 1)

* `Parametr beta` - 117716.2 (beta - 4)
...
* `Parametr alfa` - 136597.7	(alpha - 1)

* `Parametr beta` - 125936.3 (beta - 7)

* `Rho` - 131834.5	(rho - 0.5)

Dla TSP_127 przy użyciu algorytmu mrówkowego udało się uzyskać lepsze wyniki niż ten uzyskany przez Solver (140217.6).
...
<div style='text-align: justify'>
W poniższych tabelach dokonano zestawienia najlepszych wyników uzyskanych dla każdego z analizowanych algorytmów.
</div>

<br>
**Porównanie najlepszych wyników dla poszczególnych algorytmów (TSP_48)**
...
<br>
**Porównanie najlepszych wyników dla poszczególnych algorytmów (TSP_76)**
...
<br>
**Porównanie najlepszych wyników dla poszczególnych algorytmów (TSP_127)**
...
<br>
<div style='text-align: justify'>
Porównując wyniki uzyskane przy zastosowaniu poszczególnych algorytmów w kontekście problemu komiwojażera można zauważyć, że dla każdego z trzech przypadków (TSP_48, TSP_76 oraz TSP_127) osiągane wyniki w przeważającej większości były niższe od wartości uzyskanych za pomocą Solvera. Gorsze rezultaty uzyskano jedynie dla algorytmu mrówkowego i NN w przypadku TSP_48, dla algorytmu NN w przypadku TSP_76 oraz dla algorytmu symulowanego wyżarzania w przypadku TSP_127.

Dla wszystkich trzech przypadków najlepsze rezultaty spośród wszystkich rozważanych algorytmów uzyskano za pomocą algorytmu iteracyjnej wspinaczki. Wspomniany algorytm wykazał się największą skutecznością w kontekście minimalizacji długości trasy. Na drugim miejscu pod względem uzyskanych wyników znalazł się algorytm Tabu Search, którego wyniki również w każdym analizowanym przypadków były lepsze od rezultatów Sovera. Również algorytm genetyczny zwrócił lepsze wyniki w odniesieniu do tych z Solvera. Pozostałe algorytmy osiągały gorsze rezultaty.
</div>